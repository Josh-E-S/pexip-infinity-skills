---
name: pexip-pexrtc
description: Use when building a browser web app with PexRTC — Pexip's official JavaScript client API (script tag from node, no npm). Covers makeCall, connect, PIN/IDP/extension auth, onConnect stream wiring, roster (onParticipantCreate/Update/Delete), host controls (mute, kick, lock, layout, dial-out), screenshare (present/getPresentation), chat (sendChatMessage/onChatMessage), breakouts, FECC, live captions, STUN/TURN, and React integration. Triggers on `PexRTC`, `makeCall`, `connect`, `onSetup`, `onConnect`, `pexrtc.js`, `onParticipantCreate`, `disconnectParticipant`, `transformLayout`, `present`, `sendChatMessage`, `sendFECC`, `createBreakout`. Do NOT use for @pexip/infinity npm SDK (use pexip-call-lifecycle) or non-browser clients (use pexip-rest-client-api).
license: MIT
---
