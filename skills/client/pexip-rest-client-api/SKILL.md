...
description: Use when building a non-browser Pexip client — native mobile (iOS/Android), desktop app, server-side bot, or CLI tool — that joins conferences and controls meetings via raw HTTP. Covers token auth (request_token/refresh_token/release_token), PIN/IDP auth, SDP offer/answer + ICE exchange, the full SSE event stream (participant_create/update/delete, presentation, message_received, stage, live_captions), and all REST control endpoints. Triggers on `/api/client/v2/`, `request_token`, `refresh_token`, `take_floor`, `release_floor`, `message_received`, SSE event stream, native Pexip client, server-side Pexip bot. Do NOT use for browser web apps (use pexip-pexrtc) or @pexip/infinity npm SDK (use pexip-call-lifecycle).
license: MIT
---
