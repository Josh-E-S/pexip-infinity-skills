...
For correlation guidelines (e.g., matching `conversation_id` vs `call_id`) and quality scores, consult the reference file as well.
...
2. **Return 2xx quickly.** Pexip retries on non-2xx and timeout, and queues build up fast under load. Don't do heavy processing in the request handler — receive, queue, ack. See "Performance" below.
3. **Validate HTTP Basic auth** if you set `username` / `password` on the sink.
4. **Tolerate batches.** With `bulk_support=True`, the body is an `eventsink_bulk` envelope with `data` as an array. With it off, a single event. Handle both — Pexip may send singles even with bulk on.
5. **Idempotency.** Pexip retries on failure; dedupe on `(node, seq)` or on `uuid` (e.g. `call_id`, `event_type`, `time`).
6. **Detect resets.** `seq` restarts at `1` when a sink is restarted on a node. Pair an `eventsink_started` with the immediately following `seq=1` to detect a reset cleanly.

### Performance & Offloading

Per the spec, deliveries are **in-order, one-by-one** by default, so a slow receiver builds a queue on the Conferencing Node. With bulk enabled, the queue is shorter but bursts still hit the receiver. Either way:
- **Offload immediately** from the request handler to a local queue (Redis Streams / Kafka / SQS / channel + worker pool). The handler's only job is "validate auth, accept JSON, push to queue, return 200."
- **Diagnose back-pressure.** Track and alarm on receiver response-time percentiles and on the gap between Pexip's `time` field and your processing time.
- **Don't block on downstream sinks.** If a downstream system (warehouse, SIEM) is slow, the queue absorbs it. If you write synchronously to a slow downstream, you'll spread the slowness to Pexip itself.

---

## Production Webhook Receiver Patterns

Below are concrete, production-grade receiver architectures demonstrating rapid ingestion, authentication, payload unwrapping (bulk & single), idempotency checks, and handling out-of-order events.

### Pattern 1: Node.js / TypeScript (Express + Redis Queue)
This pattern achieves sub-millisecond response times by verifying Basic Auth, extracting events from the payload envelope, and offloading them to a Redis list queue while applying a distributed idempotency lock.
...
app.post('/webhook/pexip-events', basicAuth, async (req: Request, res: Response) => {
  try {
    ...
    // 1. Immediately acknowledge to Pexip (critical to prevent node queue backups)
    res.status(200).json({ status: 'accepted' });

    // 2. Normalize payload (Pexip sends either a single event envelope OR bulk array envelope)
    let events: any[] = [];
    if (payload.event === 'eventsink_bulk') {
      events = payload.data || [];
    ...
    for (const event of events) {
      // Create a unique key using the originating node IP and sequence number
      // We also include the timestamp since sequence numbers reset to 1 when a sink restarts
      const dedupeKey = `pexip:event:${event.node}:${event.seq}:${Math.floor(event.time)}`;

      // Set the key in Redis with a 24-hour expiration only if it doesn't already exist (NX)
      const isUnique = await redis.set(dedupeKey, '1', 'NX', 'EX', 86400);
      if (isUnique) {
        // Enqueue the event for background workers to process
        await redis.rpush('pexip:queue:events', JSON.stringify(event));
      } else {
        console.warn(`[Webhook] Duplicate event ignored: Node=${event.node}, Seq=${event.seq}`);
      }
    }
  } catch (error) {
    console.error('[Webhook] Error receiving events:', error);
  }
});

app.listen(3000, () => console.log('Pexip webhook receiver listening on port 3000'));
```

### Pattern 2: Python (FastAPI + BackgroundTasks)
This pattern uses FastAPI's built-in asynchronous model and `BackgroundTasks` to respond instantly to Pexip, while processing and filtering events out of the request loop thread.
...
def verify_credentials(credentials: HTTPBasicCredentials = Depends(security)):
    correct_username = secrets.compare_digest(credentials.username, WEBHOOK_USER)
    correct_password = secrets.compare_digest(credentials.password, WEBHOOK_PASS)
    if not (correct_username and correct_password):
        raise HTTPException(status_code=401, detail="Unauthorized")

def process_event_background(event: Dict[str, Any]):
    # Implements out-of-order handling using Redis to store the latest event time per resource
    resource_id = event.get("data", {}).get("participant_uuid")
    if not resource_id:
        resource_id = event.get("data", {}).get("conversation_id")

    if resource_id:
        state_key = f"pexip:state:timestamp:{resource_id}"
        event_time = float(event.get("time", 0))

        # Atomically check if the incoming event is newer than the last processed event
        last_time = r.get(state_key)
        if last_time and float(last_time) >= event_time:
            print(f"[Worker] Ignored out-of-order event for {resource_id} (Incoming time: {event_time} <= Processed: {last_time})")
            return

        # Update the timestamp
        r.set(state_key, str(event_time), ex=86400) # Expire in 24h

    # Log/Save the event (e.g., insert into Postgres/BigQuery/Elasticsearch)
    print(f"[Worker] Processed event: {event.get('event')} (Seq: {event.get('seq')})")
...
async def receive_events(request: Request, background_tasks: BackgroundTasks):
    ...
    # Immediately schedule tasks and acknowledge Pexip
    # Accept both single events and eventsink_bulk wrapper arrays
    events = []
    if isinstance(body, dict):
        if body.get("event") == "eventsink_bulk":
    ...
    for ev in events:
        node = ev.get("node")
        seq = ev.get("seq")
        time_stamp = ev.get("time")

        # Check idempotency lock
        dedupe_key = f"pexip:event:{node}:{seq}:{int(float(time_stamp))}"
        if r.set(dedupe_key, "1", nx=True, ex=86400):
            background_tasks.add_task(process_event_background, ev)
        else:
            print(f"[Webhook] Duplicate event filtered: Node={node}, Seq={seq}")
...
## Idempotency & Out-of-Order Delivery Strategies

When building any receiver, you must plan for two issues: **retry duplication** and **out-of-order delivery** (which can occur during periods of network latency or node failovers).

### 1. Guaranteeing Idempotency
- **The Compound Key**: The combination of `(node, seq, Math.floor(time))` provides a unique identifier for any single event dispatched by a Conferencing Node.
- **The Bulk Wrapper**: When `bulk_support=True` is enabled, multiple events are wrapped in a single POST. Make sure your deduplication logic evaluates each child event in the array individually, not the parent bulk wrapper itself.

### 2. Handling Out-of-Order Events
Because Conferencing Nodes process events independently, and network paths may vary, a `participant_disconnected` event could theoretically arrive before or at the same time as a delayed `participant_connected` event.
- **Timestamp Tracking**: Before applying any status changes to your system database, compare the event's `time` timestamp with the `last_updated` timestamp stored for that resource.
- **Rules**:
  - If `IncomingEvent.time < DatabaseResource.last_updated`, discard the incoming event's state (but log it if necessary for debugging).
  - Only update the state if the incoming event's timestamp is strictly newer.
