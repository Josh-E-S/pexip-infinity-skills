...
description: Use when adding, modifying, or debugging MCP tools that wrap the Pexip Infinity Management Configuration API — CRUD on VMRs/conferences, conference_alias, end_user, system_location, worker_vm (conferencing nodes), gateway_routing_rule, automatic_participant, ldap_sync_source, ivr_theme, event_sink, global settings, license, devices. Triggers on `/api/admin/configuration/v1/`, `conference`, `conference_alias`, `end_user`, `system_location`, `worker_vm`, `gateway_routing_rule`, `automatic_participant`, `ldap_sync_source`, `ivr_theme`, `event_sink`, `tools/conference.py`, `tools/alias.py`, `tools/end_user.py`, `tools/gateway_rule.py`, `tools/infrastructure.py`. Do NOT use for live in-progress meetings (use `pexip-command-api` + `pexip-status-api`) or post-call CDRs (use `pexip-history-api`).
license: MIT
---
...
| Need | Endpoint family | Method(s) |
|---|---|---|
| VMRs / Virtual Auditoriums / Gateway Services | `/configuration/v1/conference/` | CRUD |
| Conference aliases (dial strings → VMR) | `/configuration/v1/conference_alias/` | CRUD |
| End users (directory entries) | `/configuration/v1/end_user/` | CRUD |
| System locations (datacenters) | `/configuration/v1/system_location/` | CRUD |
| Conferencing nodes (workers) | `/configuration/v1/worker_vm/` | CRUD |
| Dial-plan routing rules | `/configuration/v1/gateway_routing_rule/` | CRUD |
| Automatic participants (always-on calls) | `/configuration/v1/automatic_participant/` | CRUD |
| Event sinks (webhook config) | `/configuration/v1/event_sink/` | CRUD |
| LDAP sync sources | `/configuration/v1/ldap_sync_source/` | CRUD |
| Live JSON schema introspection | `…/<resource>/schema/?format=json` | GET |

All endpoints share base URL `https://<management-node>/api/admin/configuration/v1/`, HTTP Basic auth over HTTPS, the same pagination + filtering conventions (see below), and a 1,000 req / 60 s rate limit. **Always introspect `schema/` before hand-coding fields** — fields drift between Pexip versions.
