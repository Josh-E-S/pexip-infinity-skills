...
|---|---|---|
| Completed conference instances (CDR headers) | `/history/v1/conference/` | GET |
| Completed participant call legs (CDR detail) | `/history/v1/participant/` | GET |
| Deep call-quality data for one participant | `/history/v1/participant/<id>/` | GET |
| Filter by time window | `?start_time__gte=…&start_time__lt=…` | — |
| Filter by quality / disconnect reason | `?call_quality=bad`, `?disconnect_reason=…` | — |
| Live JSON schema introspection | `…/<resource>/schema/?format=json` | GET |

All endpoints: HTTP Basic over HTTPS, **GET only**, same Tastypie-style pagination + filtering as the other admin APIs, same shared 1,000 req / 60 s rate limit. **Retention is 10,000 most-recent conference instances** — export externally for long-term retention. For live in-progress meetings, use the Status API.
