...
| Need | Endpoint family | Method |
|---|---|---|
| Active conferences (right now) | `/status/v1/conference/` | GET |
| Currently-connected participants | `/status/v1/participant/` | GET |
| Conferencing Node load + state | `/status/v1/worker_vm/` | GET |
| Backplane (cluster-internal) media stats | `/status/v1/backplane/` | GET |
| Active alarms | `/status/v1/alarm/` | GET |
| Port / licensing usage | `/status/v1/licensing/` | GET |
| Cloud-burst recommendations | `/status/v1/cloud_overflow_recommendation/` | GET |
| Per-participant media streams | `/status/v1/participant_media_stream/` | GET |
| Per-participant call quality | `/status/v1/participant_call_quality/` | GET |

All endpoints: HTTP Basic over HTTPS, same Tastypie-style pagination + filtering as the other admin APIs, same shared 1,000 req / 60 s rate limit. **Read-only** — POST/PATCH/DELETE will return 405. For state-changing actions on a live meeting, use the Command API.
...
- **For "live dashboard" style tools, prefer event sinks** (configured under Configuration API → `event_sink`) for push notifications, instead of polling Status. The Status API is fine for one-shot questions.
