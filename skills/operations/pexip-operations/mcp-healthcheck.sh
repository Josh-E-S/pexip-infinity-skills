#!/usr/bin/env bash
# MCP-HOST-SPECIFIC: this script only works when the pexip-mgmnt MCP server
# is installed locally (on PATH or at $PEXIP_MCP_DIR). If you're calling
# Pexip's REST endpoints directly from your own code, you don't need this
# script — hit the Management Node directly with HTTP Basic auth.
#
# Smoke-test that the pexip-mgmnt MCP server is reachable and authed.
#
# Usage:
#   ./mcp-healthcheck.sh                # runs against the pexip_mcp module on PYTHONPATH
#   PEXIP_MCP_DIR=/path bash ./mcp-healthcheck.sh
#
