...
- **For exports / billing,** prefer pulling history on a schedule into your own warehouse rather than relying on Pexip's retention. Pexip event sinks (push-based) are a complement — see `vmr-administration.md` for the `event_sink` tools that configure them.
