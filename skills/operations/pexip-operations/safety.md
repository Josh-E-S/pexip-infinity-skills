...
If a tool call fails because the MCP server isn't connected (no `pexip-mgmnt` server in the host's MCP config), surface that clearly — "the pexip-mgmnt MCP server isn't available in this session" — and don't try to substitute by suggesting shell commands or HTTP requests against the Pexip API. The user installed the MCP server for a reason; failure should send them back to fix the wiring.
