...
For long-lived platform configuration: VMRs (Virtual Meeting Rooms), the aliases that point at them, end users in the directory, LDAP sync, event sinks, IVR themes, and the global platform settings. Hits the Configuration API. For live calls see `live-meeting-ops.md`; for dial-plan rules see `dial-plan.md`.
...
| `*_event_sink` | `event_sink` | A webhook URL Pexip pushes conference / participant events to. |
...
## Recipe: event sinks (push-based events)

Configure a webhook URL Pexip pushes events to. Event delivery is a Pexip ↔ HTTP-listener contract — neither this package nor any MCP server *receives* events on your behalf. You register a separate HTTP listener under your URL.
...
create_event_sink(
    ...
    bulk_support=True,
    verify_tls=True,
    username="pexip", password="…", # optional HTTP Basic Pexip will send
)
```

Use case: real-time CDR / event collection without polling the History API. Pair with a small HTTP listener (out of scope for this MCP server — see the parent repo's `TODO.md` for the event-sink collector sketch).
